<template>
    <div>
        <div>
            <el-form :model="formData" inline>
                ...
                <el-form-item>
                    ...
                    <el-button @click="handleCommand('packageUpload', null)">包上传</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div id="table">
            <el-table v-loading="tableLoadingRef" :data="tableData" empty-text="无数据" height="640">
                ...
                <el-table-column prop="name" label="类型名称" width="200" align="left"/>
                <el-table-column prop="version" label="最新版本" width="200" align="left"/>
                <el-table-column prop="description" label="描述" width="420" align="left" :show-overflow-tooltip="true"/>
                ...
                <el-table-column prop="updateBy" label="操作人" width="160" align="center"/>
                <el-table-column prop="updateTime" label="操作时间" width="220" align="center"/>
        ...
        <div>
            <type-edit v-model="showEditRef" :edit-data="editDataRef" @edit-success="handleEditSuccess"></type-edit>
        </div>
        <div>
            <type-upload v-model="showPackageUploadRef" @edit-success="handleUploadSuccess"></type-upload>
        </div>
    </div>
</template>
...
import TypeEdit from "@/pages/views/server/type/TypeEdit";
import TypeUpload from "@/pages/views/server/type/TypeUpload";
...
function query() {
    ...
    serverApi.typeQuery(pagination.value.currentPage, pagination.value.pageSize, params).then(res => {
...
const showPackageUploadRef = ref(false)
...
function changeState(row, state) {
    serverApi.typeChangeState([row.id,], state)
        .then(() => {
            row.state = state
...
function handleCommand(command, row) {
    switch (command) {
        ...
        case 'packageUpload':
            editDataRef.value = row
            showPackageUploadRef.value = true
            break
        case 'enable':
            changeState(row, 1)
            break
